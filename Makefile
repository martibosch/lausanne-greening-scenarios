.PHONY: agglom_lulc agglom_landsat train_test_split response_tiles train_models
...
#### code
PREDICT_MODEL_PY := $(CODE_TREES_DIR)/predict_model.py

### rules
$(CLASSIFIED_TILES_DIR): | $(DATA_INTERIM_DIR)
	mkdir $@
$(CLASSIFIED_TILES_DIR)/%.csv: $(MODELS_DIR)/%.joblib $(PREDICT_MODEL_PY) \
	| $(CLASSIFIED_TILES_DIR)
	python $(PREDICT_MODEL_PY) $(SPLIT_CSV) $< $(CLASSIFIED_TILES_DIR) $@ \
